...
gazeboTopicList = [     '/butia_manipulation/shoulder_yaw_joint_position_controller/command',
...
def inverse_kinematics_callback(data):
    ...
    for j in range(0, len(publishers)):
        ...
        if 'shoulder_pitch' in gazeboTopicList[j]:
            shoulder_pitch_joint_down.publish(angles[j])
            shoulder_elbow_joint.publish(-angles[j])
        if 'elbow_pitch_joint' in gazeboTopicList[j]:
